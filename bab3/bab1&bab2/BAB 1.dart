...
enum Role {
  Admin,
  Customer
} //menandai peran pengguna sehingga setiap pengguna dapat memiliki hak akses yang berbeda.

// Subclass dari User (OOP)
  ...
  void addProduct(Map<String, Product> productList, Product product) {
    try {
      ...
      products = productList.values
          .toSet()
          .toList(); // Menggunakan Set untuk memastikan produk unik
...
class CustomerUser extends User {
  ...
  void viewProducts() {
    ...
    products?.forEach((product) {
      print(
          "- ${product.productName}, Harga: Rp${product.price}, Stok: ${product.inStock ? "Tersedia" : "Tidak tersedia"}");
    });
  }
}
...
void main() async {
  ...
  var outOfStockProduct =
      Product(productName: "Headphones", price: 1000000, inStock: false);
  admin.addProduct(productList,
      outOfStockProduct); // Memicu exception karena produk tidak tersedia
  // Customer User
  var customer = CustomerUser("Customer", 25);
  customer.products =
      productList.values.toList(); // Inisialisasi produk untuk customer
  customer.viewProducts();
  // Admin menghapus produk
  admin.removeProduct(productList, "Smartphone");
